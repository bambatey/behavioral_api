...
class SentenceRepository(FirestoreBaseRepository[Sentence]):
    ...
    async def find_by_context(self, context_id: str) -> List[Sentence]:
        return await self.find_by(context_id=context_id)
    ...
    async def find_one_by_context_bias_position(
        self, context_id: str, bias: str, position: int
    ) -> Optional[Sentence]:
        items = await self.find_by(context_id=context_id, bias=bias, position=position)
        return items[0] if items else None

    async def delete_by_context(self, context_id: str) -> int:
        items = await self.find_by_context(context_id)
        for s in items:
